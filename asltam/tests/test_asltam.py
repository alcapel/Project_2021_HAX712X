...
import numpy as np
import asltam.io.load_price
import asltam.io.load_dist
...
def test_get_way():
    a = load_dist().save_as_dist(index=' Nom gare ')
    assert get_way(data_dist=a, start='MONTPELLIER', target='AGDE') == ['MONTPELLIER', 'SETE', 'AGDE']


def test_average_cost():
    a = load_price().save_as_price(index=' ')
    b = load_dist().save_as_dist(index=' Nom gare ')
    assert np.allclose(average_cost(data_price=load_price.subdata_price(a, [0, 1]),
    data_dist=load_dist.subdata_dist(b, [0,1])),
...
def test_average_cost_list():
    ...
    b = load_dist().save_as_dist(index=' Nom gare ')
    assert np.allclose(
                       average_cost_list(data_price=load_price.subdata_price(a, [0, 1]),
                       data_dist=load_dist.subdata_dist(b, [0, 1])),
                       np.array([1.6/17.0]))
...
def test_get_subgraph_under_k():
    a = load_price().save_as_price(index=' ')
    g = get_subgraph_under_k(load_price.subdata_price(a, range(4)), 1)
...
def test_kmin_cost_out():
    a = load_price().save_as_price(index=' ')
    b = load_dist.save_as_dist(index=' Nom gare ')
    assert am.kmin_cost_out(data_price=a,
                            data_dist=b,
                            start='MONTPELLIER',
