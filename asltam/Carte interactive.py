...
#Modifier le nom de la colone "Nom gare" pour l'utiliser dans les deux lignes suivantes 
...
def trajet(DEPART,ARRIVEE):
    ...
    #Résolution du probléme de la distance différente entre l'aller et le retour avec une boucle 
    if geo.loc[geo['gare'] == DEPART].index[0] < geo.loc[geo['gare'] == ARRIVEE].index[0] :
        
        x =[geo['Long'][geo.loc[geo['gare'] == DEPART].index[0]], 
             geo['Latt'][geo.loc[geo['gare'] == DEPART].index[0]]]
        y =[geo['Long'][geo.loc[geo['gare'] == ARRIVEE].index[0]], 
        ...
        c = folium.Map(location = [43.1837661,3.0042121],
                      zoom_start = 10,
                      control_scale = True)  
        ...
        for i in range (0, len(coor)-1):
            ...
            with(open('test.json','+w')) as f:
                ...
                folium.Marker(coord[0][::-1], 
                              popup = DEPART, 
                              icon = folium.Icon(color = 'red', icon = 'car', prefix = 'fa')
                             ).add_to(c)
                       
                folium.Marker(coord[1][::-1], 
                              popup = ARRIVEE, 
                              icon = folium.Icon(color = 'red', icon = 'car', prefix = 'fa')
                             ).add_to(c)
        return c
        ...
        m = folium.Map(location = [43.1837661,3.0042121],
                      zoom_start = 10,
                      control_scale = True)
