...
class load_dist :
	...
	Voir aussi : load_geo, load_price.
	"""
	def __init__(self, url = url_dist, target = path_dist):
		...
		Paramètres
		----------
		url : str, url d'origine de vos données. Le paramètre
		par défaut est celui utilisé en guise d'illustration du
		package. Il télécharge les données des distances entre
		les différentes gares de péage du réseau autoroutiers de
		la région Occitanie, en France.
		
		target : str, chemin d'accès pour télécharger votre
		DataFrame. Par défaut, la classe téléchargera le fichier
		dans le repertoire courant sous le nom de 'data_dist.csv'.
		"""
	...
	def save_as_dist(path=path_dist, index=None):
		...
		Paramètres
		----------
		path : str, chemin d'accès pour lire votre DataFrame. Par
		défaut, il s'agit du chemin utilisé lors du téléchargement des
		données en ayant utilisé le paramètre par défaut.
		
		index : str, indique quel colonne vous voulez mettre en index 
		(si vous le désirez). Il ne sera pas utilisé par défaut.
		
		Return
		------
		df_dist : pandas.DataFrame
		Renvoie un pandas DataFrame avec vos données des distances.
		"""
	...
	def subdata_dist(data_dist, index):
		...
		Paramètres
		----------
		data_dist : pandas.DataFrame, une matrice de distance.
		
		index : list, liste d'index utilisé pour extraire le 
		sous tableau.
	...
	def is_dist(data_dist):
		...
		Paramètres
		----------
		data_dist : DataFrame, tableau de donnée à tester.
		
		Return
		------
		Bool, True si c'est bien un talbeau de distance,
		False sinon.
